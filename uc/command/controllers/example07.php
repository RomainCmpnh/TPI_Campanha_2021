<?php
/**
 * Html2Pdf Library - example
 *
 * HTML => PDF converter
 * distributed under the OSL-3.0 License
 *
 * @package   Html2pdf
 * @author    Laurent MINGUET <webmaster@html2pdf.fr>
 * @copyright 2017 Laurent MINGUET
 */
require_once dirname(__FILE__).'/../../../vendor/autoload.php';
require_once 'uc/panier/models/commandItem.php';
require_once 'uc/command/model/command.php';
require_once 'uc/items/models/item.php'; 
require_once 'commons/views/Html.php';

use Spipu\Html2Pdf\Html2Pdf;
use Spipu\Html2Pdf\Exception\Html2PdfException;
use Spipu\Html2Pdf\Exception\ExceptionFormatter;

try {
    // get the HTML
    ob_start();
    include dirname(__FILE__).'../../views/example07a.php';
    include dirname(__FILE__).'../../views/example07b.php';
    $content = ob_get_clean();

    // $idCommand = filter_input(INPUT_GET, 'idCommand', FILTER_VALIDATE_INT);
    // $commandItem = commandItem::FindByCommandId($idCommand);
    // //$command = command::FindById($idCommand);
    // $idItem = $commandItem->getIdItem();
    // $item = Item::FindById($idItem);
    // $user = Session::getUser();
    $idCommand = filter_input(INPUT_POST, 'idCommand', FILTER_VALIDATE_INT);
    
    
    $html2pdf = new Html2Pdf('P', 'A4', 'fr');
    $html2pdf->pdf->SetDisplayMode('fullpage');
    $html2pdf->writeHTML($content);
    ob_end_clean();
    if (is_int($idCommand)) {
        $command = command::findById($idCommand);
        }
    Html::showHtmlPage('pdfPanier', $html2pdf->output('example07.pdf'), array("command" => $command));
    
} catch (Html2PdfException $e) {
    $html2pdf->clean();

    $formatter = new ExceptionFormatter($e);
    echo $formatter->getHtmlMessage();
}
